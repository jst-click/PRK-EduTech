...
class MyApp extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return MaterialApp(
      ...
      home: const YouTubeLiveClassPage(),
    );
  }
}

class YouTubeLiveClassPage extends StatefulWidget {
  const YouTubeLiveClassPage({Key? key}) : super(key: key);
...
class _YouTubeLiveClassPageState extends State<YouTubeLiveClassPage> {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        setState(() {
          ...
          applyFilters(); // Apply filter immediately after fetching
          isLoading = false;
        });
      } else {
        setState(() {
  ...
  void applyFilters() {
    setState(() {
      filteredVideos = videos.where((video) => video.isYoutubeLive).toList();
    });
  }
